...
//   };
const paymentsData = [
    { id: "#PAY-001", orderId: "#ORD-001", amount: 1299.00, method: "Credit Card", status: "Completed", date: "2023-10-01" },
    { id: "#PAY-002", orderId: "#ORD-002", amount: 1199.00, method: "PayPal", status: "Pending", date: "2023-10-02" },
    { id: "#PAY-003", orderId: "#ORD-003", amount: 249.00, method: "Debit Card", status: "Failed", date: "2023-10-03" },
    { id: "#PAY-004", orderId: "#ORD-004", amount: 599.00, method: "Credit Card", status: "Completed", date: "2023-10-04" },
    { id: "#PAY-005", orderId: "#ORD-005", amount: 399.00, method: "Bank Transfer", status: "Completed", date: "2023-10-05" },
];
      ...
      >
        ...
        <div className="p-4 md:p-6 lg:p-8">
                    ...
                    <Card className="bg-white/5 border-white/10 shadow-lg mt-6">
                        <CardHeader>
                            <CardTitle className="text-lg font-semibold text-white">Payment Transactions</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader className="bg-white/5">
                                    <TableRow className="border-white/10 hover:bg-white/5">
                                        <TableHead className="text-white/70 text-md">Payment ID</TableHead>
                                ...
                                <TableBody>
                                    {filteredPayments.map(payment => (
                                        <TableRow key={payment.id} className="border-white/10 hover:bg-white/5">
                                            <TableCell className="text-white/70 text-md">{payment.id}</TableCell>
                                            <TableCell className="text-white/70 text-md">{payment.orderId}</TableCell>
                                            <TableCell className="text-white/70 text-md">Rs {payment.amount.toFixed(2)}</TableCell>
                                            <TableCell className="text-white/70 text-md">{payment.method}</TableCell>
                                            ...
                                            <TableCell>
                                                <Badge className={`
                                                ${payment.status === "Completed" ? "bg-green-500/20 text-green-400 hover:bg-green-500/30" : 
                                                payment.status === "Pending" ? "bg-yellow-500/20 text-yellow-400  hover:bg-yellow-500/30" : 
                                                "bg-red-500/20 text-red-400 hover:bg-red-500/30"}`}>{payment.status}

                                                </Badge>

                                            </TableCell>
                                            <TableCell className="text-white/70 text-md">{payment.date}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
